					...
					<pg:pager>
						<pg:index export="totalItems = itemCount">
							<div class="pager">
								<pg:first>
									<a class="start"
										href="getAllContractConsumerBills?currPage=1&theMonth=<s:property value='theMonth'/>&ordertype=<s:property value="ordertype"/>">首页</a>
								</pg:first>
								<pg:prev>
									...
									<s:else>
										<a class="prev"
											href="getAllContractConsumerBills?currPage=<s:property value="prevPage"/>&theMonth=<s:property value="theMonth"/>&ordertype=<s:property value="ordertype"/>">上一页</a>
									</s:else>
								</pg:prev>
								<pg:pages>
								...
								<pg:next>
									...
									<s:else>
										<a class="next"
											href="getAllContractConsumerBills?currPage=<s:property value="nextPage"/>&theMonth=<s:property value="theMonth"/>&ordertype=<s:property value="ordertype"/>">下一页</a>
									</s:else>
								</pg:next>
								<pg:last>
									<a class="end"
										href="getAllContractConsumerBills?currPage=<s:property value="totalPages"/>&theMonth=<s:property value="theMonth"/>&ordertype=<s:property value="ordertype"/>">末页</a>
								</pg:last>
							</div>
							<!-- /.pager -->
						</pg:index>
	...
	<script type="text/javascript">
		$(function(){
			$('#filt').bind('click', function(e){
				...
				phoneNo = $('#phone').val();
				window.location.href = "getAllContractConsumerBills?theMonth=" + theMonth + "&phoneNo=" + phoneNo + "&ordertype="+order;
			});
		});
