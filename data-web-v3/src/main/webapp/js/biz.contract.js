...
function initDataTable() {
    if ($('#J-table-contract').length > 0) {
        /*$('#J-table-contract tfoot th').each(function() {
            var title = $('#J-table-contract thead th').eq($(this).index()).text();
            $(this).html('<input type="text" width="10px" placeholder="Search ' + title + '" />');
        });*/

        maintain_table = $('#J-table-contract').dataTable({
            ...
            "bDeferRender": true,
            "iDisplayLength": 10,
            "sAjaxSource": '/api/consumers/contract',
            "bProcessing": true,
            ...
            "oLanguage": {
                ...
                "sInfoEmpty": "<span style='color:red'>未查询到6个月内合约到期的用户</span>",
