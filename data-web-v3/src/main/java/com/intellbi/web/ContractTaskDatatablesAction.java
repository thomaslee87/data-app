...
public class ContractTaskDatatablesAction extends ActionSupport{
	...
	//            int recordsTotal = consumerBillService.getTotalCount(theMonth, phoneNo,userId,-1);
	            ...
	            if(taskview.equals(Constants.TASK_VIEW_MONTH)){
	            	int cConsumerCnt = consumerBillService.getContractCnt(userId, theMonth, cQueryWindow, -1, hideDone, theBizMonth);
		            dataMap.put("recordsTotal", String.valueOf(cConsumerCnt));
		            dataMap.put("recordsFiltered", String.valueOf(cConsumerCnt));
	            	dataMap.put("data", getDataList(userId,-1, bizdate,theMonth,cQueryWindow,orderby,iDisplayStart,iDisplayLength,hideDone, theBizMonth));
	            }
	            //打个补丁，日/周视图推荐
		            ...
		            dataMap.put("data", getDataList(userId, 0, bizdate,theMonth,cQueryWindow,orderby,0,5,hideDone, theBizMonth));
		            ...
		            dataMap.put("data", getDataList(userId, 0, bizdate,theMonth,cQueryWindow,orderby,iDisplayStart,iDisplayLength,hideDone,theBizMonth));
	            }
...
*/	}
	...
	private List<List<Object>> getDataList(int userId, int contractTaskState, String bizdate,String theMonth, Period cQueryWindow, String orderby, int pageStart, int pageSize, int hideDone, String theBizMonth
	        ){
        ...
        try {
            ...
            List<ConsumerBillDO> bills = consumerBillService.getContractConsumers(userId, theMonth, cQueryWindow, pagination, contractTaskState, hideDone,theBizMonth);
...
//                _dtList.add(consumerBillDO.getPhoneNo());
                ...
                phoneMap.put("isphoto", String.valueOf(consumerBillDO.getIsPhoto()));
...
//                	_dtList.add("<span id=\"btn" + consumerBillDO.getPhoneNo() + "\" class=\"btn btn-warning btn-xs\" value=\""+consumerBillDO.getPhoneNo()+"\">未处理</span>");
                /*else if(consumerBillDO.getTaskStateContract() == 1)
                	_dtList.add("<span class=\"label label-success\">已查看</span>");*/
                else if(consumerBillDO.getTaskStateContract() == 1)
                    _dtList.add("<span class=\"label label-success\">已处理</span>");
...
//                _map.put("isphoto", String.valueOf(consumerBillDO.getIsPhoto()));
                _dtList.add(_map);
