...
function initDataTable() {
    if ($('#J-table-bandwidth').length > 0) {
        /*$('#J-table-bandwidth tfoot th').each(function() {
            var title = $('#J-table-bandwidth thead th').eq($(this).index()).text();
            $(this).html('<input type="text" width="10px" placeholder="Search ' + title + '" />');
        });*/

        bandwidth_table = $('#J-table-bandwidth').dataTable({
            ...
            "bDeferRender": true,
            "iDisplayLength": 10,
            "sAjaxSource": '/api/consumers/bandwidth',
            "bProcessing": true,
            ...
            "aoColumns": [
                ...
                }, 
                {
                    ...
                    "mRender":function(mData){
                           ...
                           return '<a href="bandwidth/billdetail.html?theBizMonth='+ mData.month+'&phoneNo='+mData.phone +'" target=_blank class="btn btn-primary btn-xs" data-toggle="popover-left" ' +
                            'data-content="' + sv + '元" ' + 
            ...
            "fnServerData": function(sSource, aoData, fnCallback) {
                $.ajax({
                    ...
                    "data": {
                        ...
                        "orderby": function(){
                            var columns = ['phone_no', 'value_change_4g', 'income', 'income6','gprs','gprs6'];
                            var flag = [0, 0, 0, 0,0,0];
                            var order = ['asc', 'asc', 'asc', 'asc', 'asc', 'asc']
                            var sortCols = aoData['iSortingCols'];
                            for (var i = 0; i < sortCols; i++) {
                                flag[aoData['iSortCol_' + i] ] = 1;
                                order[aoData['iSortCol_' + i] ] = aoData['sSortDir_' + i];
                            }
                            var orderby = '';
